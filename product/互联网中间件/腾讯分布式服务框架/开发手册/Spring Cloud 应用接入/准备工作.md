为了更好地将云中间件的功能以云服务的方式提供给大家，TSF 对 Spring Cloud 的一些组件进行了替换，增强了安全性和易用性。
...
参考 [《Maven 配置云 TSF 仓库地址》](/document/product/649/19018)，在 Demo 工程 `pom.xml` 所在目录执行 `mvn clean package` 即可下载 TSF SDK。
